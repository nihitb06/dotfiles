...
local function create_button(symbol, color, hover_color, cmd, key)
    ...
    helpers.add_hover_cursor(icon, "hand2")
...
local brave = create_button("", x.color5, x.color13, apps.brave, "b")
local neofetch = create_button("", x.color4, x.color12, apps.neofetch, "n")
local mail = create_button("", x.color6, x.color14, apps.mail, "m")
local files = create_button("", x.color3, x.color11, apps.file_manager, "f")
local terminal = create_button("", x.color5, x.color13, apps.terminal, "t")
local youtube = create_button("", x.color1, x.color9, apps.youtube, "y")
local networks = create_button("", x.color3, x.color11, apps.networks, "=")
local reddit = create_button("", x.color1, x.color9, apps.reddit, "r")
local night_mode = create_button("", x.color1, x.color9, apps.night_mode, "x")
local popcorntime = create_button("", x.color4, x.color12, apps.popcorntime, "p")
local github = create_button("", x.color6, x.color14, apps.github, "g")
local spotify = create_button("", x.color2, x.color10, apps.spotify, "s")
...
app_drawer:setup {
    ...
    {
        -- Stripes
        create_stripe({browser, brave, popcorntime, spotify}, "#00000000"),
        create_stripe({terminal, files, editor, neofetch}, x.color8.."20"),
        create_stripe({youtube, github, reddit, mail}, x.color8.."40"),
