...
mpd_prev_icon:buttons(gears.table.join(
    ...
    awful.button({ }, 3, function ()
        ...
        awful.spawn.with_shell("playerctl position 10-")
    end)
))
...
awesome.connect_signal("evil::spotify", function(artist, title, status)
    ...
    if status == "paused" then
        ...
        if artist ~= last_artist and title ~= last_title then
            accent = music_playing_colors[(music_playing_counter % #music_playing_colors) + 6]
...
helpers.add_hover_cursor(mpd_next_icon, "hand2")
helpers.add_hover_cursor(mpd_prev_icon, "hand2")
helpers.add_hover_cursor(mpd_toggle_icon, "hand2")
